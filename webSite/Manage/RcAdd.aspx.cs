...
public partial class Manage_RcInfo_Add : System.Web.UI.Page
{
    private int rcId;
    ...
    {
        DWGX.Model.RcInfo rcInfoModel = new DWGX.Model.RcInfo();
        if (rcInfoModel != null)
        {
            try
            {
                ...
                rcInfoModel.createTime = DateTime.Now;
                rcInfoModel.updateTime = DateTime.Now;

                new DWGX.BLL.RcInfo().Add(rcInfoModel);
                Response.Write(new CommonClass().MessageBox("保存成功", "RcList.aspx"));
            }
            catch
            {
                Response.Write(new CommonClass().MessageBox("保存失败,请检查内容是否有非法值"));
            }
